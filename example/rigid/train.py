...
checkpoint = {
    'state': False, 
    'path': 'outputs/checkpoint_36.pth', 
    'num_start': 36, 
    'last_episode': 6351
}
episodes = 50000
...
coef = {
    ...
    "coef_distance_target": -5.0,   # 惩罚到目标点的距离
    "coef_distance_no_safty": -15.0,  # 惩罚不安全距离
    "coef_crash": -20.0,    # 惩罚碰撞
    "coef_get_target": 25.0,    # 奖励到达目标位置
    "coef_alive": 1.0,  # 奖励存活
}
# act = torch.tensor(
#     [
...
cylinder_dict = {'num':20, 'r_min':1.0, 'r_max':5.0, 'z_min':0.3, 'z_max':0.3, 'R_min':0.1, 'R_max':0.3}
...
""" 模型初始化 """
model = model.Model_pos().to(device)
...
# 加载checkpoint
if checkpoint['state']:
    model_checkpoint = torch.load(checkpoint['path'])
    # 按顺序恢复状态
    model.load_state_dict(model_checkpoint['model_state_dict'])
    optim.load_state_dict(model_checkpoint['optimizer_state_dict'])
    start_episode = model_checkpoint['epoch'] + 1  # 从下一轮开始
    print(f'Loaded checkpoint from epoch {model_checkpoint["epoch"]}')
    checkpoint_num = checkpoint['num_start']
    last_checkpoint_episode = checkpoint['last_episode']
for episode in range(start_episode, episodes):
    ...
    for step in range(steps):
        ...
        # 随机目标位置
        for i in range(batch_size):
            mask = torch.norm(obs['pos'][i,0,:]-target_pos, dim=-1) < 0.1
            if mask[i,:]:
                theta = random.uniform(0, 2*math.pi)
                target_pos[i, :, 0] = r*math.cos(theta)
                target_pos[i, :, 1] = r*math.sin(theta)
        ...
        target_distance = torch.norm((obs['pos']-target_pos), dim=-1)
        vel_to_pt = ((distance_prev-obs['distance'])*75).clamp_min(1.0)
        delta_distance = (safty_distance-obs['distance']).clamp_min(0.0)
        is_robot_get_target = target_distance <= 0.2
        # 计算奖励
        reward = (
            coef["coef_out_vel"]*vel_delta + \
            coef["coef_distance_target"]*target_distance + \
            coef["coef_distance_no_safty"]*vel_to_pt*delta_distance.squeeze(1) + \
